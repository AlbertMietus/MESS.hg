...
.. 1200 words ex sidebar (a bit long)
...
.. sidebar:: Maps & Milestones
   ...
   **LSP** became an open standard in 2016; in the same year, an early Emacs plugin became available (`lsp-mode`). Now
   superseded by `eglot`, which started in 2018 and has been a part of Emacs since 2023.
   |BR|
...
mistakes*, and we require certainty. Code that mostly uses register ``Y`` and *only* at one place uses pin ``X`` isn't
...
*training phase* --not an option when it isn't open-source. But even then, it knows only the old code!

So, our LLM needs a bit of help. To prevent it from using a statistical approach (aka *guessing*), it should have access to
your code. Conceptually, you can add all your code as *context* --but that’s overkill. A more meticulous procedure would
be beneficial.
|BR|
The *old* LSP protocol has proven its use. It is fully deterministic, has ‘life’ access to the current version, and
...
That is why we need to comprehend LSP before we can study :ref:`MCP <CodeAI-MCP>`. See you soon, plz keep using own synapses --ALbert
