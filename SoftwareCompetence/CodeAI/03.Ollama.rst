...
.. sidebar:: Maps & Milestones
   ...
   transition path: start small and local, try less-local bigger models, and invest in your own hardware when it works
   and you need maximum sovereignty.

   Ollama is the best-known tool for local, safe, sovereign LLMs, but not the only one. It is based on `Llama.cpp
   <https://en.wikipedia.org/wiki/Llama.cpp>`__, a library that comes with its own CLI tool, and used by other tools as
   well.
   |BR|
   Apple's 'MLX' can also be mentioned here. It used to be an alternative, but is now integrated in Ollama. Especially
...
to an LLM, again and again. The same applies to your design and other IP you give to the AI-tool. And, as most cloud tools
...
*alias* to the latest version. Until recently...
|BR|
...
Sure, I could have pinned to an *x.y* version; but I like to use the latest verified one (as we all are used to). But
``-latest`` isn't verified; it's just the latest, not the greatest.
|BR|
...
them on the developer's laptop. Both work, with pros and cons. Usually, the PC-local one is faster; the (on-premises)
server-local one can run bigger, more powerful LLMs. Both are safe and give you sovereignty by using the normal
network security you should have installed anyway.
...
that needs (e.g.) Excel to process it.
|BR|
Only an LLM is bigger: even a small one is often about 10 Gigabytes! This implies you need a lot of RAM.


Some LLMs are commercial, but there are also open-source (and/or open-weight) LLMs. You can download them and use them
...
<https://github.com/ollama/ollama>`__ --- so you can even inspect it for quality and security issues (if you want to go
the last mile).
